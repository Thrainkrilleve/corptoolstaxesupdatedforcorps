...
class CharacterRattingTaxConfiguration(models.Model):
    ...
    def __str_console__(self) -> str:
        ...
        if not len(regions):
            regions = "Everywhere"
        return f"{self.name} \033[31m{self.tax:,.2f}%\033[39m in:\033[32m{regions}\033[39m"

    def get_payment_data(self, start_date=MIN_DATE, end_date=MAX_DATE, alliance_filter=None, corp_filter=None):
    ...
    def get_payment_data_from_ids(self, entry_ids, alliance_filter=None, corp_filter=None):
        query = CharacterWalletJournalEntry.objects.filter(
            ref_type__in=["bounty_prizes"])
        if alliance_filter:
            query = query.filter(
    ...
    def process_character_aggregates(self, data):
        ...
        for d in data:
            if d['entry_id'] not in trans_ids:
                ...
                except (TypeError, KeyError, decimal.InvalidOperation) as e:
    ...
    def get_character_aggregates(self, start_date=MIN_DATE, end_date=MAX_DATE, alliance_filter=None, corp_filter=None):
        data = self.get_payment_data(start_date, end_date, alliance_filter, corp_filter).values(
    ...
    def get_character_aggregates_id(self, ids, alliance_filter=None, corp_filter=None):
        data = self.get_payment_data_from_ids(ids, alliance_filter=alliance_filter, corp_filter=corp_filter).values(
            'amount',           # Player Payment
            'tax',              # Tax Amount
            'tax_receiver_id',  # corp that got taxed
            'entry_id',         # Unique to not double charge
    ...
    def get_character_aggregates_corp_level(self, start_date=MIN_DATE, end_date=MAX_DATE, full=True, alliance_filter=None, corp_filter=None):
        logger.debug(
            f"TAXTOOLS: Started get_character_aggregates_corp_level {self.__str_discord__()}")
        data = self.get_character_aggregates(
            start_date, end_date, alliance_filter, corp_filter)
        return self.process_character_aggregates_corp_level(data, full)

    def get_character_aggregates_corp_level_id(self, ids, start_date=MIN_DATE, end_date=MAX_DATE, full=True, alliance_filter=None, corp_filter=None):
        logger.debug(
            f"TAXTOOLS: Started get_character_aggregates_corp_level {self.__str_discord__()}")
        data = self.get_character_aggregates_id(
            ids, alliance_filter, corp_filter)
        return self.process_character_aggregates_corp_level(data, full)
...
class CharacterPayoutTaxConfiguration(models.Model):
    ...
    def get_payment_data(self, start_date=MIN_DATE, end_date=MAX_DATE, alliance_filter=None, corp_filter=None):
        ref_types = [rt.strip() for rt in self.wallet_transaction_type.split(",")]
        query = CharacterWalletJournalEntry.objects.filter(
            date__gte=start_date,
            date__lte=end_date,
    ...
    def get_character_aggregates_ids(self, ids, alliance_filter=None, corp_filter=None):
        logger.debug(
            f"TAXTOOLS: Started get_character_aggregates {self.__str_console__()}")
        data = self.get_payment_data_from_ids(ids, alliance_filter, corp_filter).values(
            'amount',
            'entry_id',
            'date',
            'tax',
    ...
    def get_character_aggregates_corp_level_id(self, ids, full=True, alliance_filter=None, corp_filter=None):
        logger.debug(
...
class CorpTaxPayoutTaxConfiguration(models.Model):
    ...
    def get_payment_data(self, start_date=datetime.min, end_date=datetime.max, alliance_filter=None, corp_filter=None):
        """
        Retrieve corporate wallet journal entries for tax calculation.
        
        Args:
            start_date: Start of period to query
            end_date: End of period to query
            alliance_filter: List of alliance IDs to filter by (optional)
            corp_filter: List of corporation IDs to filter by (optional)
            
        Important: Corporations without alliance membership (alliance_id=None) will
        NOT match alliance_filter and must be explicitly included via corp_filter.
        
        Returns:
            QuerySet of CorporationWalletJournalEntry objects
        """
        query = CorporationWalletJournalEntry.objects.filter(
            date__gte=start_date,
            date__lte=end_date,
        ...
        if alliance_filter:
            query = query.filter(
                division__corporation__corporation__alliance_id__in=alliance_filter)
        if corp_filter:
            query = query.filter(
                division__corporation__corporation__corporation_id__in=corp_filter)
        
        return query

    def get_aggregates(self, start_date=datetime.min, end_date=datetime.max, full=True, alliance_filter=None, corp_filter=None):
        output = {}
        tax_cache = {}
        trans_ids = set()
        for w in self.get_payment_data(start_date=start_date, end_date=end_date, alliance_filter=alliance_filter, corp_filter=corp_filter):
...
class CorpTaxPerMemberTaxConfiguration(models.Model):
    ...
    def get_main_counts(self, alliance_filter=None, corp_filter=None):
        ...
        if alliance_filter:
            characters = characters.filter(
                character_ownership__user__profile__main_character__alliance_id__in=alliance_filter)
        if corp_filter:
            characters = characters.filter(
                character_ownership__user__profile__main_character__corporation_id__in=corp_filter)
    ...
    def get_invoice_data(self, alliance_filter=None, corp_filter=None):
        logger.debug(
            f"TAXTOOLS: Started get_invoice_data {self.__str_discord__()}")
        corp_list = self.get_main_counts(alliance_filter=alliance_filter, corp_filter=corp_filter)
        corp_info = {}
        output = {}
...
class CorpTaxPerServiceModuleConfiguration(models.Model):
    ...
    def get_service_counts(self, alliance_filter=None, corp_filter=None):  # TODO update
        services = [s.strip() for s in self.module_filters.split(",")]
        ...
        if alliance_filter:
            structures = structures.filter(
                corporation__corporation__alliance_id__in=alliance_filter)
        if corp_filter:
            structures = structures.filter(
                corporation__corporation__corporation_id__in=corp_filter)
    ...
    def get_invoice_data(self, alliance_filter=None, corp_filter=None):  # TODO update
        logger.debug(
            f"TAXTOOLS: Started get_invoice_data {self.__str_discord__()}")
        corp_list = self.get_service_counts(alliance_filter=alliance_filter, corp_filter=corp_filter)
        output = {}
...
class CorpTaxConfiguration(models.Model):
    ...
    exempted_corps = models.ManyToManyField(EveCorporationInfo, blank=True, related_name='tax_exemptions')

    included_alliances = models.ManyToManyField(EveAllianceInfo, blank=True)
    included_corporations = models.ManyToManyField(EveCorporationInfo, blank=True, related_name='tax_inclusions')
    ...
    def calculate_tax(self, start_date=datetime.min, end_date=datetime.max, alliance_filter=None, corp_filter=None):
        """
        ...
        for tax in self.character_ratting_included.all():
            _taxes = tax.get_character_aggregates_corp_level(
                start_date=start_date, end_date=end_date, alliance_filter=alliance_filter, corp_filter=corp_filter)
        ...
        for tax in self.corporate_member_tax_included.all():
            _taxes = tax.get_invoice_data(alliance_filter=alliance_filter, corp_filter=corp_filter)
    ...
    def get_invoice_data(self):
        """
        Extract invoice data for the current tax period.
        
        This method automatically:
        1. Calculates the date range (last invoice date to now)
        2. Extracts alliance and corporation filters from the configuration
        3. Calls calculate_tax() with appropriate filters
        
        Filter Behavior:
            - included_alliances: If set, only corps in these alliances are taxed
            - included_corporations: If set, only these specific corps are taxed
            - Both: Corps matching EITHER criterion are taxed (OR logic)
            - Neither: ALL corps with taxable activity are taxed
            
        Note: Corporations without alliance membership (alliance_id=None) will NOT
        match alliance filters and must be explicitly added to included_corporations.
        
        Returns:
            Tuple of (start_date, end_date, tax_calculation_results)
        """
        ...
        if self.included_corporations.all().count():
            corps = self.included_corporations.all().values_list("corporation_id", flat=True)
            logger.debug(f"TAXTOOLS: Included corporations: {list(corps)}")

        return start_date, end_date, self.calculate_tax(start_date=start_date, end_date=end_date, alliance_filter=alliances, corp_filter=corps)
    ...
    def send_invoices(self):
        ...
        for id, tax in taxes['taxes'].items():
            ...
            except (AssertionError, ObjectDoesNotExist, AttributeError) as e:
                logger.error(f"TAXTOOLS: send_invoices offset_any_credits failed for corp {id}")
    ...
    def rerun_taxes(self, record_id: int, alliance_filter=None, corp_filter=None):
        """
        Recalculate taxes for a previously processed tax record.
        
        This is useful for adjusting taxes after configuration changes or corrections.
        Uses the same transaction IDs from the original record.
        
        Args:
            record_id: ID of the CorpTaxRecord to recalculate
            alliance_filter: List of alliance IDs to include (optional)
            corp_filter: List of corporation IDs to include (optional)
            
        Note: Filters use OR logic - corporations matching EITHER criterion are included.
        
        Returns:
            Dictionary containing recalculated taxes and transaction IDs
        """
        ...
        for tax in self.character_ratting_included.all():
            _taxes = tax.get_character_aggregates_corp_level_id(
                record_data['char_trans_ids'], alliance_filter=alliance_filter, corp_filter=corp_filter)
            output["char_tax"].append(_taxes)
            for cid, data in _taxes.items():
                if cid not in excluded_cids:
                    amount = round(data['tax_to_pay'], -6)
